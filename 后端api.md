...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name | String | 是 | 收藏夹名称（1-50字符，同一用户下唯一） |
...
```json
{
  ...
  "data": [
    ...
    },
    {
      ...
      "updateTime": "2026-04-27T11:00:00"
    }
  ]
}
```

> **FavoriteFolderVO 字段说明**:
> - `id`: 收藏夹 ID
> - `userId`: 所属用户 ID
> - `name`: 收藏夹名称
> - `description`: 收藏夹描述
> - `sortOrder`: 排序序号
> - `articleCount`: 该收藏夹下的文章数量
> - `createTime`: 创建时间
> - `updateTime`: 更新时间
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name | String | 是 | 新的收藏夹名称（1-50字符，同一用户下唯一） |
...
> ⚠️ 删除收藏夹会同时删除该收藏夹下的所有收藏记录（即取消该收藏夹下所有文章的收藏）。
...
| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| folderIds | Long[] | 是 | 排序后的收藏夹 ID 数组（按期望的顺序排列） |

---

## 📋 数据模型（补充）

### FavoriteFolderVO（收藏夹展示对象）
```json
{
  "id": 1,
  "userId": 1,
